...
| GET | `/api/v1/news/assets/{asset_id}` | Noticias | Consultar noticias de un activo | noticias.leer | - | NewsListResponse | asset_id (path, req); start_at (query, opt); end_at (query, opt); limit (query, opt); offset (query, opt); order_by (query, opt) | 200, 422 |

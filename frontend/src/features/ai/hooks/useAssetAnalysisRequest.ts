...
export function useAssetAnalysisRequest(requestId: string | null, enabled = true) {
  return useQuery({
    queryKey: aiQueryKeys.assetAnalysisRequest(requestId ?? 'none'),
    queryFn: () => {
      if (!requestId) {
        throw new Error('Se requiere una solicitud para consultar el análisis')
      }
